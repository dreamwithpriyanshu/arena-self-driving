...
function addMetric(record) { metrics.push(record); drawCharts(); metricsText.textContent = `Episode ${record.episode_num}: reward ${Number(record.total_reward).toFixed(2)}, steps ${record.steps}, ε ${Number(record.epsilon).toFixed(3)}`; }
...
async function launchSession(mode) { const response = await fetch(`/sessions/${mode}/start`, {method:'POST'}); const data = await response.json(); sessionStatus.textContent = response.ok ? `${mode === 'human' ? 'Human drive' : 'SARSA playback'} opened in a native PyGame window.` : (data.detail || 'Could not open native window.'); }
